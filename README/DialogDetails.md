...
第一个顶部按钮名（topButtonName）String类型；第二个，顶部按钮颜色（topColor）String类型；
第三个，中间按钮名（midButtonName）String类型；第四个，中间按钮颜色（midColor）String类型；
第五个，取消按钮名（cancelButtonName）String类型；第六个，取消按钮颜色（cancelColor）String类型。实现 OnBottomDialogDetailClick 接口。具体使用见下方代码范例。
```
