...
#### 使用方式:
- 极简使用：传俩个变量，实现一个接口。
第一个，顶部按钮名（topButtonName）String类型；第二个，中间按钮名（midButtonName）String类型；第三个，实现 OnBottomDialogClick 接口。
...
- 高级使用：传六个变量，实现一个接口。
该种使用，不推荐直接使用，需要按照各自的需求简单封装后，才适宜使用。
第一个顶部按钮名（topButtonName）String类型；第二个，顶部按钮颜色（topColor）String类型；第三个，中间按钮名（midButtonName）String类型；第四个，中间按钮颜色（midColor）String类型；第五个，取消按钮名（cancelButtonName）String类型；第六个，取消按钮颜色（cancelColor）String类型。实现 OnBottomDialogDetailClick 接口。具体使用见下方代码范例。
```
